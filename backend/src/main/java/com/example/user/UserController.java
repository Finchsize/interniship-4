...
public class UserController {
    ...
    public String loginUser(@RequestBody User user) {
        ...
        try {
            String token = JWT.create().withClaim("id", specifiedUser.get().getId()).withIssuedAt(new Date()).withExpiresAt(new Date(System.currentTimeMillis() + 5000L)).sign(jwtAlgorithm);
            return token;
